        ...
        return Column(children: [
          PopupMenuItem(
            onTap: () {
              if (ref.watch(editingMessageDetails).content.isNotEmpty) {
                ...
                ref.read(replayIsMineProvider.notifier).state = false;
                ref.read(replayIsMineProvider.notifier).state = false;
                ref.read(messageIndexProvider.notifier).state =
                    replayMessageIndex;
          ...
          if (isMe)
            if (isMessageText)
              PopupMenuItem(
                onTap: () async {
                  if (ref.watch(replayProvider).isNotEmpty) {
                    ref.read(replayProvider.notifier).state = '';
                    ref.read(replayIsMineProvider.notifier).state = false;
                    ref.read(messageIndexProvider.notifier).state =
                        replayMessageIndex;
                    ...
                    ref.read(editingMessageDetails.notifier).state = message;
                  } else {
                    ref.read(messageIndexProvider.notifier).state =
                        replayMessageIndex;
                    ref.read(emojiShowingProvider.notifier).state = false;
          ...
          PopupMenuItem(
            onTap: () {
              showPopover(
                  ...
                  bodyBuilder: (context) => Column(
                          ...
                          children: [
                            if (message.receiverId == userId)
                              PopupMenuItem(
                                  onTap: () {
                                    ref
                                        .read(
                                            deleteEditMessagesProvider.notifier)
                                        .deleteSingleMessageForAll(
                                            ...
                                            '${message.sentTime}');
                                    ref
                                        .read(showMenuOpenedProvider.notifier)
                                        .state = false;
                                  },
                                  child: Text(
                            ...
                            PopupMenuItem(
                                onTap: () {
                                  ref
                                      .read(deleteEditMessagesProvider.notifier)
                  ...
                  width: message.receiverId != userId ? 140 : 250,
                  height: message.receiverId == userId ? 140 : 60,
                  arrowHeight: 15,
                  arrowWidth: 30,
